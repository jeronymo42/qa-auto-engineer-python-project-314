...
def test_new_user_create(user_page:UsersPage):
    ...
    user_row = user_page.find_table_row_by_data(page_header='Users', data=TEST_USER_EMAIL)
...
def test_delete_label(labels_page:LabelsPage):
    ...
    assert labels_page.is_invisible(
        (By.XPATH, f"//td[text()='{old_name_1}']"))

@pytest.mark.smoke    
@pytest.mark.tasks
def test_open_create_status_form(tasks_page:TasksPage):
...
def test_create_simple_task_status(tasks_page:TasksPage):
    ...
    STATUS = 'To Be Fixed'
    tasks_page.open_create_element_form()
    tasks_page.fill_task(ASSIGNEE, TASK_TITLE, f'Description of task {TASK_TITLE}', STATUS)
...
def test_delete_task(tasks_page:TasksPage):
    ...
    assert tasks_page.is_not_located((By.XPATH, f"//div[@role='button']//div[text()='{TITLE}']"))
