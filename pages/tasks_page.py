...
class TasksPage(BasePage, TasksPageLocators):
    ...
    STATUS_MAP = {'Draft':1, 'To Review':2,
                     'To Be Fixed':3, 'To Publish':4, 'Published':4}
    ...
    def is_task_in_column(self, task_title, column_name):
        ...
        return len(column_elem.find_elements(By.XPATH, f".//div[text()='{task_title}']"))

    def filter_by(self, filter_type, filter_value):
        self.driver.find_element(*self.FILTER_TYPES[filter_type]).click()
        goal_item = filter(lambda x: x.text == filter_value, self.driver.find_elements(*self.FILTER_LIST_ITEM))
    ...
    def delete_task(self, task_title):
        self.driver.find_element(By.XPATH, f"//div[text()='{task_title}']/../..//a[@aria-label='Edit']").click()
        self.driver.find_element(*self.DELETE_BUTTON).click()
        self.wait.until(EC.invisibility_of_element_located(self.ELEMENT_DELETED_MESSAGE))

    def find_column_by_number(self, number):
        return self.driver.find_element(By.XPATH, f"//div[@data-rfd-droppable-id='{number}']")
    ...
    def move_card(self, task_title, column_name):
        ...
        ActionChains(self.driver) \
            .drag_and_drop_by_offset(task, finish['x'] - start['x'], finish['y'] - start['y']) \
            .perform()
        return self
